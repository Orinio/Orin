...
exports.buildDailyTipPrompt = buildDailyTipPrompt;
exports.buildWeeklyInsightPrompt = buildWeeklyInsightPrompt;
exports.buildMilestonePrompt = buildMilestonePrompt;
exports.buildAdHocPrompt = buildAdHocPrompt;
exports.buildOnboardingPrompt = buildOnboardingPrompt;
exports.getPromptForNoteType = getPromptForNoteType;
exports.parseCoachResponse = parseCoachResponse;
...
function buildDailyTipPrompt(context) {
...
function buildWeeklyInsightPrompt(context) {
    const { user, skillAnalysis, proofs } = context;
    const verifiedCount = proofs.filter((p) => p.verificationStatus === 'verified').length;
...
function buildMilestonePrompt(context) {
...
function buildAdHocPrompt(context, userQuery) {
    const { user, skillAnalysis, proofs } = context;
    const verifiedCount = proofs.filter((p) => p.verificationStatus === 'verified').length;
    const topSkills = skillAnalysis.topSkills.slice(0, 5).map((s) => s.name).join(', ');
    const skillGaps = skillAnalysis.skillGaps.slice(0, 5);
...
function buildOnboardingPrompt(context) {
    const { user } = context;
...
function getPromptForNoteType(context, userQuery) {
