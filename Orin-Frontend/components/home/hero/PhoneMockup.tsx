...
interface FloatingCardProps {
  children: ReactNode;
  className?: string;
  position: 'top-left' | 'bottom-right' | 'center-right';
  delay?: string;
}

/* ── FloatingCard ──────────────────────────────────────── */
function FloatingCard({ children, className = '', position }: FloatingCardProps) {
  const positionStyles = {
    'top-left': 'top-6 -left-2 md:-left-12',
    'bottom-right': 'bottom-20 -right-2 md:-right-10',
    'center-right': 'top-1/2 -right-2 md:-right-14 hidden lg:block',
  };

  return (
    <div
      className={`absolute z-30 ${positionStyles[position]} ${className}`}
      aria-hidden="true"
    >
      {children}
    </div>
...
function AICoachCard() {
  return (
    <FloatingCard position="top-left" className="animate-float-slow" delay="0.5s">
      <div
...
function PhoneScreen() {
  return (
    <>
      ...
      <div
        className="flex justify-between items-center px-5 pt-2.5 pb-1 text-[10px] font-semibold"
        style={{ color: 'var(--color-ink)' }}
      >
        <span>9:41</span>
        <div className="flex items-center gap-1">
          <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
            <path d="M1 9l2 2c4.97-4.97 13.03-4.97 18 0l2-2C16.93 2.93 7.08 2.93 5 13z" />
          </svg>
          <div
            className="w-5 h-2.5 border rounded-sm relative"
            style={{ borderColor: 'var(--color-ink)' }}
          >
            <div
              className="absolute inset-0.5 rounded-[1px]"
              style={{ backgroundColor: 'var(--color-ink)', width: '80%' }}
            />
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(180deg, var(--color-surface) 0%, var(--color-paper) 100%)',
        }}
      >
        <AppHeader />
        <HeroCard />
    ...
    <div className="flex items-center justify-between mb-3 animate-slideInUp">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p
            className="text-[8px] font-semibold mt-0.5 flex items-center gap-1"
            style={{ color: 'var(--color-bloom)' }}
          >
            <span
...
function BottomNav() {
  return (
    <div
      className="mt-2 mx-auto w-fit px-1.5 py-1 rounded-full flex items-center gap-1 shadow-lg"
      style={{ backgroundColor: 'var(--color-ink)' }}
    >
      <NavDot active />
      <NavDot />
...
export default function PhoneMockup() {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="ORIN mobile app mockup showing proof cards, AI coach, and job opportunities"
    >
      {/* Background glow */}
      ...
      <div
        ref={containerRef}
        className="relative z-20 animate-float-slow transition-transform duration-300 ease-out"
