...
exports._clearDeduplicationCache = _clearDeduplicationCache;
...
function requestDeduplication(req, res, next) {
    ...
    if (lastSeen && now - lastSeen < DEDUP_WINDOW_MS) {
        logger_js_1.logger.warn({ fingerprint, lastSeen: now - lastSeen, requestId: req.id }, 'Duplicate request rejected');
        res.status(409).json({
...
function _clearDeduplicationCache() {
