...
function getModelForUseCase(useCase) {
    const category = exports.MODELS[useCase];
    if (!category || typeof category !== 'object') {
        return exports.MODELS.primary.chat; // Fallback
    }
    return category.primary || Object.values(category)[0];
}
/**
 * Get all available models as an array
 */
function getAllModels() {
    return Object.entries(exports.MODEL_METADATA).map(([id, metadata]) => ({
        id,
        metadata
    }));
}
...
function supportsToolCalling(modelId) {
    return modelId === exports.MODELS.toolCalling.primary ||
        modelId === exports.MODELS.toolCalling.fallback;
}
/**
 * Get the best model for a given criteria
 */
function getBestModel(criteria) {
    const models = Object.entries(exports.MODEL_METADATA);
    let filtered = models;
    if (criteria.toolCalling) {
        filtered = filtered.filter(([_, m]) => m.toolCalling);
    }
    if (criteria.multimodal) {
        filtered = filtered.filter(([_, m]) => m.multimodal);
    }
    ...
    if (criteria.speed) {
        ...
        filtered.sort((a, b) => {
            const aSpeed = speedOrder[a[1].speed] ?? 2;
            const bSpeed = speedOrder[b[1].speed] ?? 2;
            return aSpeed - bSpeed;
        });
    }
    return filtered[0]?.[0] || exports.MODELS.primary.chat;
}
